...
class TitleState extends MusicBeatState
{
	...
	var easterEggKeys:Array<String> = [
		'SHADOW', 'RIVER', 'SHUBS', 'BBPANZU', 'SENBEN'
	];
	...
	function startIntro()
	{
		if (!initialized)
		{
			if(FlxG.sound.music == null) {
				switch(music){
					...
					case "pause":
						FlxG.sound.playMusic(Paths.formatToSongPath(ClientPrefs.data.pauseMusic));
				}
			}
		}
		...
		switch(easterEgg.toUpperCase())
		{
			...
			case 'SENBEN':
				gfDance.frames = Paths.getSparrowAtlas('MeBump');
				gfDance.animation.addByIndices('danceLeft', 'Me Title Bump', [0,1,2,3,4], "", 24, false);
				gfDance.animation.addByIndices('danceRight', 'Me Title Bump',[4,5,6,7,8], "", 24, false);
			#end
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 6:
					#if PSYCH_WATERMARKS
					createCoolText(['This is a story in my school'], -40);
					#else
					//createCoolText(['In association', 'with'], -40);
					#end
				case 8:
					addMoreText('I love my mod\n orign from', -40);
					ngSpr.visible = true;
				// credTextShit.text += '\nNewgrounds';
				case 9:
					deleteCoolText();
		...
		if (!skippedIntro)
		{
			if (playJingle) //Ignore deez
			{
				...
				switch(easteregg)
				{
					...
					case 'SENBEN':
						sound = FlxG.sound.play(Paths.sound('JingleMe'));
